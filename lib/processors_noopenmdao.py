...
class findFaceGetPulse(object):
    ...
    def run(self, cam):
        ...
        if self.find_faces:
            ...
            cv2.putText(self.frame_out, "Press 'P' to enter phone number for alerts",
        ...
        '''
        if key == 80:
            phone_number= cv2.putText(
            self.frame_out, input('Enter your phone number, press E to submit') ,
            (10, 100), cv2.FONT_HERSHEY_PLAIN, 1.25, col)
            if key == 69:
                destroyWindow(self.plot_title)
                cv2.putText(
                self.frame_out, "The number you entered is",phone_number,"is that correct?(Y/N)",
                   (10, 100), cv2.FONT_HERSHEY_PLAIN, 1.5, col)
        '''             
